...
class InventoryTracker:
    ...
    def update_quantity(self, item_search, quantity_sold):
        ...
        if len(matching_items) > 0:
            for item_name, quantity in matching_items:
                ...
                item_data = [[transferred_item, str(transferred_quantity)]]  # Make item_data a 2D list
    ...
    def restock_item(self, item_model, additional_quantity):
        ...
        data = self.Stock.get_all_values()
        for index, row in enumerate(data[1:], start=2):  # Start from index 1 to skip the header row
            if row[0] == item_model:
